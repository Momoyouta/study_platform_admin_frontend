import type {
  CourseOutlineChapterDto,
  CourseOutlineDraftContent,
  CourseOutlineDraftDto,
  CourseOutlineLessonDto,
  PublishIdMappingsDto,
} from '@/type/course';
...
export const createOutlineQuickUpdatePayload = (
  draft: CourseOutlineDraftDto,
  chapterId: string,
  lessonId: string,
  patch: Partial<CourseOutlineLessonDto>
) => {
  const nextDraft = draft.chapters.map((chapter) => {
