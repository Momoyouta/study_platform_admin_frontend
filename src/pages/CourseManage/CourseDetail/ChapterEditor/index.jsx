...
import {
  ...
  publishCourseOutlineAdmin,
  saveCourseDraftAdmin,
  updateChapterTitleQuickAdmin,
  updateLessonQuickAdmin,
} from '@/http/api.ts';
...
const CourseOutline = ({ courseId }) => {
  ...
  const handlePublish = () => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        setPublishing(true);
        try {
          const res = await publishCourseOutlineAdmin({
            course_id: courseId,
            draft_content: draftContent,
          });
  ...
  const handleRenameChapter = async (chapterId, newTitle) => {
    ...
    try {
      ...
      const res = await updateChapterTitleQuickAdmin({
        course_id: courseId,
  ...
  const handleSaveLessonImmediate = async (updatedLesson) => {
    ...
    try {
      const res = await updateLessonQuickAdmin({
        course_id: courseId,
        draft_content: nextDraft,
        lesson: {
