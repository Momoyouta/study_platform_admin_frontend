...
import {
    bindTeachingGroupTeachersAdmin,
    createCourseInviteAdmin,
    createTeachingGroupAdmin,
    deleteTeachingGroupAdmin,
    getTeachingGroupAdmin,
    listTeachingGroupAdmin,
    querySchoolTeacherByNameAdmin,
    updateTeachingGroupAdmin,
} from '@/http/api.ts';
import './index.less';
...
const getInviteMeta = (record) => {
    const code = record?.invitation_code || null;
    ...
    const expireUnix = createUnix && ttl ? createUnix + ttl : null;
...
const convertSecondsToUnitValue = (seconds) => {
    ...
    return {
        value: Math.max(1, Math.round(ttl / 60)),
        unit: 'minute',
    };
};
...
const TeachingGroupManage = ({ courseId, schoolId }) => {
    ...
    const [inviteExpireUnit, setInviteExpireUnit] = useState('hour');
    ...
    const currentSchoolId = useMemo(() => {
        ...
        return mySchoolId || schoolId;
    }, [isPlatformAdmin, mySchoolId, schoolId]);
    ...
    const fetchList = useCallback(async (pageParams, searchValues) => {
        ...
        setLoading(true);
        try {
            const res = await listTeachingGroupAdmin(params);
    ...
    const queryTeachersByName = useCallback(async (keyword) => {
        ...
        try {
            const res = await querySchoolTeacherByNameAdmin({
    ...
    const openEditModal = async (record) => {
        ...
        try {
            const res = await getTeachingGroupAdmin(String(record.id));
            if (!isSuccessResponse(res)) {
                return;
            }
            const detail = res?.data || {};

            const detailTeacherSelection = getTeacherSelection(detail);
            const detailTeacherIds = detailTeacherSelection.teacherIds.length
                ? detailTeacherSelection.teacherIds
                : teacherIds;
            const detailTeacherNames = detailTeacherSelection.teacherNames.length
                ? detailTeacherSelection.teacherNames
                : teacherNames;

            groupForm.setFieldsValue({
                name: detail?.name || record?.name,
                teacher_ids: detailTeacherIds,
            });
            appendTeacherOptionsBySelection(detailTeacherIds, detailTeacherNames);
        } catch (error) {
            console.error('Failed to fetch teaching group detail', error);
        }
    };
    ...
    const handleSaveGroup = async () => {
        try {
            ...
            if (editingGroup) {
                const updateRes = await updateTeachingGroupAdmin({
                ...
                const createRes = await createTeachingGroupAdmin({
                    course_id: String(courseId),
                    name: values.name,
                });
            ...
            if (teachingGroupId && teacherIds.length) {
                const bindRes = await bindTeachingGroupTeachersAdmin({
                    course_id: String(courseId),
    ...
    const handleDeleteGroup = (record) => {
        Modal.confirm({
            ...
            onOk: async () => {
                try {
                    const res = await deleteTeachingGroupAdmin(String(record.id));
                    if (!isSuccessResponse(res)) {
    ...
    const openInviteModal = (record) => {
        ...
        const converted = convertSecondsToUnitValue(inviteMeta.ttl || 86400);

        setInviteExpireUnit(converted.unit);
        inviteForm.setFieldsValue({
            expire_value: converted.value,
        });
    ...
    const handleRegenerateInvite = async () => {
        if (!inviteTarget?.id || !courseId) {
            message.error('缺少必要参数，无法生成邀请码');
            return;
        }
        ...
        try {
            ...
            const expireMultiplier = unitMap[inviteExpireUnit] || 3600;
            ...
            const payload = {
                course_id: String(courseId),
                teaching_group_id: String(inviteTarget.id),
            };

            if (expireSeconds) {
                payload.ttl = expireSeconds;
            }
            ...
            const res = await createCourseInviteAdmin(payload);
            if (!isSuccessResponse(res) && !res?.data?.code) {
    ...
    return (
        <div className="teaching-group-manage">
            ...
            <Modal
                ...
                onOk={handleRegenerateInvite}
                onCancel={() => {
                    setInviteModalOpen(false);
                    setInviteTarget(null);
                    setInviteExpireUnit('hour');
                    inviteForm.resetFields();
                }}
                okText="确定"
                cancelText="取消"
                ...
                destroyOnClose
            >
                <div className="invite-modal-group-text">
                    当前教学组：{inviteTarget?.name || inviteTarget?.id || '-'}
                </div>
                <Form form={inviteForm} layout="vertical">
                    <Form.Item
                        label="邀请码有效期"
                        extra="可选，必须大于0。留空则使用后端默认值"
                    >
                        <Space.Compact style={{ width: '100%' }}>
                            ...
                            <Select
                                value={inviteExpireUnit}
                                onChange={setInviteExpireUnit}
                                style={{ width: '30%' }}
                                options={[
